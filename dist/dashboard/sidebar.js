...
export function initSidebar() {
    const jd = el('jdInput');
    const keywords = el('keywordsInput');
    const boolean = el('booleanRule');
    const country = el('countryFilter');
    const searchBtn = el('searchBtn');
    const searchStatus = el('searchStatus');
    const themeSelect = el('themeSelect');
    const scanPages = el('scanPages');
    const aiKey = el('aiKey');
    const aiFast = el('aiFast');
    const aiSmart = el('aiSmart');
    const templateSelect = el('templateSelect');
    const saveTemplateBtn = el('saveTemplateBtn');
    const loadTemplateBtn = el('loadTemplateBtn');
    const deleteTemplateBtn = el('deleteTemplateBtn');
    const setStatus = (msg) => {
    ...
    jdDrop.addEventListener('dragleave', (e) => {
        ...
        if (!jdDrop.contains(e.relatedTarget))
    ...
    const searchQuery = () => {
        ...
        if (jd.value.trim())
            return extractKeywordsFromJD(jd.value);
        return '';
    };
    const runSearch = () => {
        ...
        if (boolean.value.trim()) {
            try {
                ...
                setStatus('❌ Invalid Boolean rule: ' + e.message);
                return;
            }
        }
        const maxPages = clampScanPages(parseInt(scanPages.value, 10));
        searchBtn.disabled = true;
        setStatus(maxPages > 1 ? 'Opening search — scanning up to ' + maxPages + ' pages…' : 'Opening search…');
        // newTab: run in a dedicated LinkedIn tab so the dashboard isn't navigated away.
        chrome.runtime.sendMessage({ type: MESSAGE.START_SEARCH, data: { query, maxPages, newTab: true } }, (response) => {
