...
| 2026-09-05 | claude/chrome-extension-architecture-cj7pul | BUGFIX ROUND (v1.30.0): (1) CRITICAL DATA LOSS — scoringEngine.persistScoresLocal wrote the run's map as the WHOLE profileScores, so any subset run (dashboard 'Retry failed', 'Score selected', or Score in a folder/shortlist view) erased every other candidate's score. New pure `shared/resultMerge.ts` mergeIntoStored() is now the persistence contract; scoringEngine seeds from storage before the first persist (seedScoresFromStorage) and aiEvalEngine routes through the same helper so the two can't drift. +4 tests. (2) Restore now truly REPLACES as its confirm promises — keysToClearOnRestore() clears workspace keys the backup lacks (aiKey untouched; stale lastRemoved dropped). +2 tests. (3) storage.onChanged reload is debounced (STORAGE_RELOAD_DEBOUNCE_MS) so bursts + a local write's duplicate render collapse into one. (4) Dashboard rows now use shared nameFormat.nameFromUrl — 'Sarah Chen' instead of the raw slug, matching popup + CSV/Excel. (5) Minor: aiUsage re-read per profile so a mid-run 'Reset counters' isn't clobbered; dead EXTRACT_NOW handler + constant removed; popup toggleShortlist read-modify-writes so it can't revert dashboard changes; searchSession uses lastFocusedWindow (service workers have no 'current window'). | done (unmerged) |
| 2026-08-26 | claude/chrome-extension-architecture-cj7pul | HANDOVER UPDATE (docs only): rewrote HANDOVER.md — backlog A–E now all merged (v1.29.1); doc reframed to 'status & next steps'. Open items are owner-only: Chrome Web Store submission (PRIVACY.md/docs ready; fill contact email + screenshots) and the CI-if-it-recurs note. Added a quick module map + future-work ideas. | done |
...
| 2026-08-26 | claude/chrome-extension-architecture-cj7pul | WORKSPACE BACKUP (v1.29.0): new pure `shared/workspaceBackup.ts` (serializeWorkspace/parseWorkspace, versioned `gazy-workspace` envelope, tolerant validator) — export the whole workspace (profiles/scores/aiEvals/shortlist/folders/templates/settings) to a JSON file and restore it, so data survives a profile reset/reinstall. The DeepSeek `aiKey` and ephemeral `lastRemoved` are deliberately EXCLUDED. Dashboard toolbar gains 💾 Backup + ⟳ Restore (file input, confirm before overwrite). +7 unit tests + a Playwright e2e that round-trips a download and asserts the key is never exported. | done (unmerged) |
| 2026-08-26 | claude/chrome-extension-architecture-cj7pul | HANDOVER NOTE: added `HANDOVER.md` — prioritised remaining backlog for the next account to pick up (A: workspace export/import JSON [recommended next]; B: a11y + keyboard shortcuts + run report; C: Chrome Web Store readiness + privacy policy; D: typed storage accessor; E: dashboard render efficiency). Also flags the open owner action: GitHub Actions not auto-running on main pushes since going public → Settings → Actions → General → "Allow all actions". Docs only, no version bump. | done |
| 2026-08-26 | claude/chrome-extension-architecture-cj7pul | RELIABILITY BUNDLE (v1.28.0): (1) DeepSeek 429/5xx retry — `retry.ts` gains isRetryableHttpStatus([429,500,502,503,504]); deepseek.evaluateProfile now retries rate-limits + transient 5xx + network errors up to AI_RETRY_COUNT with exponential backoff, honouring Retry-After; non-retryable (401/400) fails at once. (2) Empty-page diagnostic — new pure `shared/pageDiagnostics.ts` diagnoseEmptyPage(signals) distinguishes logged-out / layout-changed / genuinely-empty; content extractor.pageSignals() gathers signals (loginWall/hasResultsContainer/matched/onSearchPage), threaded via PAGE_EXTRACTED → searchSession surfaces the specific message. (3) Dashboard "↻ Retry failed (N)" toolbar button re-scores only ⚠️ failed scrapes in the current view (pure rows.failedScrapeUrls). +7 unit tests, dashboard e2e extended for the retry button. Content scripts stay classic (no ES module syntax; verified). | done (unmerged) |
...
| 2026-08-24 | claude/chrome-extension-architecture-cj7pul | XLSX export (v1.24.0): new shared/xlsx (pure, AutoFilter, +tests), candidateExport buildCandidateSheet + ext param, dashboard + popup Excel buttons, e2e empty-state assertion loosened                                                                                                                                                                                                                               | merged (PR #45) |
| 2026-08-26 | claude/chrome-extension-architecture-cj7pul | FOLDERS ARE PERSISTENT SAVES (v1.25.0): dashboard removal is now view-aware — All-view Remove drops from results but KEEPS folder/shortlist membership; folder-view Remove unfiles from that folder only; shortlist-view Remove un-stars. Row universe = profiles ∪ folder members ∪ shortlist; inView('all')=results only. New folders.removeUrlsFromFolder (+test). Undo unchanged (snapshot-driven) + count label. | done (unmerged) |

### 2026-08-26 — folders are persistent saves (branch `claude/chrome-extension-architecture-cj7pul`)

Owner request: "removing from the dashboard shouldn't remove from the folder —
only removing inside the folder should."

- Dashboard rows are now built from the **universe** = working results (`profiles`)
  ∪ all folder members ∪ shortlist, so a candidate saved to a folder keeps
  rendering there even after it leaves the results list. `inView('all')` is the
  results set only (was: everything).
- **Remove is view-aware** (`removeSelected` dispatches):
  - All/results view → `removeFromResults`: drops from `profiles` only; folders +
    shortlist untouched; scores/AI pruned only for candidates no longer saved
    anywhere.
  - Folder view → `removeFromFolderView`: unfiles from THAT folder only.
  - Shortlist view → `removeFromShortlistView`: un-stars only.
- New pure `folders.removeUrlsFromFolder(store, name, urls)` (+test). `clearAll`
  now calls `removeFromResults`. Undo is snapshot-driven as before (added a
  `count` for the status label). The popup's ✕ already only dropped from
  `profiles`, so it's consistent with no change.
