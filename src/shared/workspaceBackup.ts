...
export type WorkspaceData = Partial<Record<WorkspaceKey, unknown>>;
...
interface Envelope {
  ...
  version: number;
  exportedAt: string;
  app: string; // human-facing app version at export time (informational)
  data: WorkspaceData;
}
...
function pickKnown(source: Record<string, unknown>): WorkspaceData {
 ...
 */
export function serializeWorkspace(data: Record<string, unknown>, appVersion = ''): string {
...
export interface ParsedWorkspace {
  data: WorkspaceData;
  exportedAt: string;
  /** Number of recognised keys the file carried. */
  keyCount: number;
}
...
export function parseWorkspace(text: string): ParsedWorkspace {
  ...
  if (typeof env.version !== 'number' || env.version > FORMAT_VERSION) {
    throw new Error(
      'This backup was made by a newer version of Gazy. Update the extension, then import again.',
    );
  }
 ...
 */
export function keysToClearOnRestore(data: WorkspaceData): WorkspaceKey[] {
