 ...
 */
export function readPdfString(s: string, start: number): { str: string; next: number } {
  let i = start + 1;
  let depth = 1;
  let out = '';
  ...
  while (i < s.length) {
    ...
    if (c === '\\') {
      ...
      if (d in SIMPLE) {
        ...
        let oct = d;
        let k = i + 2;
        while (k < s.length && k < i + 4 && s[k] >= '0' && s[k] <= '7') oct += s[k++];
...
export function pdfContentToText(content: string): string {
  ...
  while (i < n) {
    ...
    if (c === '(') {
      ...
      let j = i + 1;
      while (j < n && (content[j] === '.' || (content[j] >= '0' && content[j] <= '9'))) j++;
      const num = parseFloat(content.slice(i, j));
      if (!Number.isNaN(num) && num <= TJ_SPACE_THRESHOLD) out += ' ';
      i = j;
    } else {
      i++;
    }
